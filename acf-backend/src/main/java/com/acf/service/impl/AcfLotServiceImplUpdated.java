...
import com.acf.mapper.AcfTransactionMapperCustom;
...
public class AcfLotServiceImplUpdated extends ServiceImpl<AcfLotMapper, AcfLot> implements AcfLotService {

    private final AcfMaterialService materialService;
    private final AcfTransactionMapperCustom transactionMapper;
    ...
    public void returnStock(String lotNumber, BigDecimal quantity, String warehouseLocation,
        ...
        // 查找最后一次发料记录以计算回温时间
        LocalDateTime lastOutboundTime = transactionMapper.getLastOutboundTime(lotNumber);
        LocalDateTime warmupEndTime = LocalDateTime.now();
        Integer warmupDuration = null;

        if (lastOutboundTime != null) {
            warmupDuration = (int) ChronoUnit.MINUTES.between(lastOutboundTime, warmupEndTime);
        }

        // 恢复库存
        lot.setCurrentQuantity(lot.getCurrentQuantity().add(quantity));
        lot.setWarehouseLocation(warehouseLocation);
        lot.setLotStatus("IN_STOCK");
        lot.setUpdateTime(LocalDateTime.now());
        updateById(lot);
        ...
        transaction.setOperatorId(operatorId);
        transaction.setOperatorName(operatorName);
        transaction.setWarmupEndTime(warmupEndTime);
        transaction.setWarmupDuration(warmupDuration);
        transaction.setTransactionTime(LocalDateTime.now());
        transaction.setCreateTime(LocalDateTime.now());
        transaction.setDeleted(0);
